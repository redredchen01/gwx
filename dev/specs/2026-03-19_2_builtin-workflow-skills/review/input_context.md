# Spec Review Input Context
review_type: spec_review
scope: spec

## Review Standards
Spec Review: completeness (DoD testable, AC covers happy+error, dependencies clear), tech compliance (3-layer arch, naming), codebase consistency, risk assessment, S0 criteria coverage.
Severity: P0=blocking arch/security, P1=logic/missing validation/DoD untestable, P2=naming/style.
Output: CR-{severity}-{NNN} findings. Decision: pass/conditional_pass/fix_required. 繁體中文.
...
13 個 read-only 工具，命名慣例 `workflow_{name}`：
...
> 所有 MCP workflow 工具皆為 **read-only**：FA-D 動作型只回傳預覽，不執行動作。
...
- **DoD**:
  ...
  - [ ] 超過 50 列時硬拒絕，錯誤訊息含 `exceeds limit 50`
  - [ ] `--execute` 實際發信
  - [ ] 每封信獨立失敗不影響其餘
  - [ ] `go vet` 通過
- **驗收方式**: 測試 50 列限制 + 預覽 JSON
...
- **Agent**: go-expert
- **依賴**: T-05~T-16
- **描述**: 新建 `internal/mcp/tools_workflow.go`，定義 13 個 MCP 工具（見 4.4 MCP 工具定義表）。每個工具呼叫對應 workflow 的 Run 函式，傳入 `ExecuteOpts{IsMCP: true}` 確保不執行動作。沿用 `BatchTools()` / `CallBatchTool()` 相同模式：匯出 `WorkflowTools() []Tool` 和 `CallWorkflowTool(ctx, name, args) (*ToolResult, error, bool)`。
- **DoD**:
  - [ ] 13 個工具定義完整（Name, Description, InputSchema, Required）
...
- **DoD**:
  - [ ] ListTools 回傳包含 13 個 workflow 工具
...
| # | 場景 | Given | When | Then | 優先級 |
|---|------|-------|------|------|--------|
| M-1 | MCP workflow tool 列表 | MCP server 啟動 | ListTools | 包含 13 個 `workflow_*` 工具 | P0 |
...
[truncated - see full s0_brief_spec.md]
