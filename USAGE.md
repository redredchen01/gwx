...
steps:                        # (必填) 至少一個步驟
  - id: step1                 # (選填) 步驟 ID，省略會自動產生 step_1, step_2...
    tool: gmail_search        # (必填) 要呼叫的 MCP tool 名稱，或 "transform" 偽工具
...
`transform` 不呼叫 MCP，而是在本地做資料轉換。需要 `input` 參數：
...
| 操作 | 說明 | 範例 |
|------|------|------|
| `pick` | 從每個元素只保留指定欄位 | `pick: "from,subject"` |
| `flatten` | 攤平 array of arrays | `flatten: "true"` |
| `sort_by` | 依欄位排序 | `sort_by: "date"` |
| `limit` | 取前 N 筆 | `limit: "5"` |
| `count` | 回傳列表長度 | `count: "true"` |

可以組合使用，執行順序：`pick` → `flatten` → `sort_by` → `limit` → `count`。
...
# 列出所有 skills
gwx skill list
# → {"skills": [{"name": "morning-brief", "version": "1.0", ...}, ...], "count": 17}

# 查看 skill 結構
gwx skill inspect morning-brief

# 驗證自己寫的 YAML
gwx skill validate my-workflow.yaml

# 從 GitHub Gist 安裝
gwx skill install https://gist.github.com/user/abc123

# 從本地檔案安裝
gwx skill install ~/Downloads/my-skill.yaml

# 移除
gwx skill remove my-skill
```

Skills 不透過 CLI 直接執行 — 它們自動註冊為 MCP 工具。執行方式有兩種：

1. **MCP**：Claude 直接呼叫 `skill_morning-brief`（帶 JSON 參數）
2. **Bash**：其他 Agent 透過 MCP server 呼叫
...
# → {"valid": true, "name": "invoice-finder", "inputs": 2, "steps": 4}
```
...
gwx 隨附 17 個 skill，放在 `./skills/` 目錄：

| Skill | 說明 |
|-------|------|
| `morning-brief` | 每日簡報（未讀信 + 行程 + 待辦） |
| `client-360` | 客戶 360 度視圖（信件 + 檔案 + 聯絡人） |
| `invoice-log` | 搜尋發票信件 |
| `invoice-tracker` | 搜尋發票並記錄到 Sheet |
| `seo-daily` | SEO 每日快照（Search Console + GA4） |
| `meeting-notes` | 會議記錄整理 |
| `email-digest` | 信件智慧摘要 |
| `drive-audit` | Drive 最近異動審計 |
| `sheet-compare` | 比較試算表兩個分頁 |
| `contact-export` | 匯出聯絡人 |
| `task-report` | 待辦事項報告 |
| `chat-summary` | Chat 訊息摘要 |
| `ga4-realtime` | GA4 即時數據 |
| `doc-from-sheet` | 從 Sheet 生成文件 |
| `parallel-search` | 並行搜尋 Gmail + Drive |
| `batch-notify` | 從 Sheet 批量發送個人化信件 |
| `hello` | Demo skill（列出信件 + 行程） |
